...
{
private:
	ATL::CComPtr<IOPCGroupStateMgt2>	iStateManagement2;
...
public:
	COPCGroup(const CString & groupName, bool active, unsigned long reqUpdateRate_ms, unsigned long &revisedUpdateRate_ms, float deadBand, const DWORD keepAlive, DWORD& revisedKeepAlive, COPCServer &server);

	virtual ~COPCGroup();


	COPCItem * addItem(const CString &itemName, bool active);
