...
COPCGroup::COPCGroup(const CString & groupName, bool active, unsigned long reqUpdateRate_ms, unsigned long &revisedUpdateRate_ms, float deadBand, const DWORD keepAlive, DWORD& revisedKeepAlive, COPCServer &server):
name(groupName),
opcServer(server)
{
	USES_CONVERSION;
	...
	HRESULT result = opcServer.getServerInterface()->AddGroup(wideName, active, reqUpdateRate_ms, 0, 0, &deadBand,
		0, &groupHandle, &revisedUpdateRate_ms, IID_IOPCGroupStateMgt2, (LPUNKNOWN*)&iStateManagement2);
	if (FAILED(result))
	{
		throw OPCException("Failed to Add group");
	} 

	iStateManagement2->SetKeepAlive(keepAlive, &revisedKeepAlive);

	result = iStateManagement2->QueryInterface(IID_IOPCSyncIO, (void**)&iSychIO);
	if (FAILED(result)){
		throw OPCException("Failed to get IID_IOPCSyncIO");
	}

	result = iStateManagement2->QueryInterface(IID_IOPCAsyncIO2, (void**)&iAsych2IO);
	if (FAILED(result)){
		throw OPCException("Failed to get IID_IOPCAsyncIO2");
	}

	result = iStateManagement2->QueryInterface(IID_IOPCItemMgt, (void**)&iItemManagement);
	if (FAILED(result)){
...
COPCItem * COPCGroup::addItem(const CString &itemName, bool active)
{
	...
	if (addItems(names, itemsCreated, errors, active)!= 0){
		throw OPCException("Ошибка при добавлении тега");
	}
...
int COPCGroup::addItems(CAtlArray<CString>& itemName, CAtlArray<COPCItem *>& itemsCreated, CAtlArray<HRESULT>& errors, bool active)
{
	...
	//////////////////////////////////////////////////////////////////////////
	//HRESULT	result = iStateManagement->QueryInterface(IID_IOPCItemMgt, (void**)&iItemManagement);
	//iItemManagement->AddItems(noItems, itemDef, &itemDetails, &itemResult);

	COPCServer& serv = getServer();
	ServerStatus status;
	serv.getStatus(status);
	//////////////////////////////////////////////////////////////////////////
...
void COPCGroup::enableAsynch(IAsynchDataCallback* handler)
{
	...
	HRESULT result = iStateManagement2->QueryInterface(IID_IConnectionPointContainer, (void**)&iConnectionPointContainer);
	if (FAILED(result))
	{
...
{
	HRESULT result = iStateManagement2->SetState(&reqUpdateRate_ms, &returnedUpdateRate_ms, &active,0, &deadBand,0,0);
	if (FAILED(result))
	{
