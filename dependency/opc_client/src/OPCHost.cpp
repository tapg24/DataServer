...
#include "OPCHost.h"
#include "OPCServer.h"
#include "../../opc_foundation/OpcEnum.h"
...
{
	// делаем доступ при вводе логина и пароля
	/*
	COAUTHIDENTITY authIdent;
	ZeroMemory(&authIdent, sizeof(COAUTHIDENTITY));
	authIdent.User = (USHORT*)L"OPCUser";
	authIdent.Domain = (USHORT*)L"SIB";
	authIdent.Password = (USHORT*)L"123";
	authIdent.UserLength = wcslen((wchar_t*)authIdent.User);
	authIdent.DomainLength = wcslen((wchar_t*)authIdent.Domain);
	authIdent.PasswordLength = wcslen((wchar_t*)authIdent.Password);
	...
	*/
	...
	//athn.pAuthIdentityData = &authIdent;
...
CLSID CRemoteHost::GetCLSIDFromRemoteRegistry(const CString & hostName, const CString &progID)
{
   //CString keyName = "SOFTWARE\\Classes\\" + progID + "\\Clsid";
	CString keyName = progID + "\\Clsid";
   HKEY remoteRegHandle;
   HKEY keyHandle;
   char classIdString[100];
   CLSID classId;
   //HRESULT result = RegConnectRegistry(hostName, HKEY_LOCAL_MACHINE, &remoteRegHandle);
   HRESULT result = RegConnectRegistry(hostName, HKEY_CLASSES_ROOT, &remoteRegHandle);

   throw OPCException("GetCLSIDFromRemoteRegistry", result);
...
COPCServer * CRemoteHost::connectDAServer(const CString & serverProgID)
{
	...
	*/	
	...
	//const WCHAR* progID = (const WCHAR*)serverProgID;
	CLSID clsid;

	HRESULT res = iCatInfo->CLSIDFromProgID(serverProgID.AllocSysString(), &clsid);
...
COPCServer * CLocalHost::connectDAServer(const CString & serverProgID)
{
	...
	MULTI_QI mqi = { &IID_IOPCServer, 0, 0 };
	result = CoCreateInstanceEx( clsid, 0,CLSCTX_ALL, 0, 1, &mqi );
	if(FAILED(result))
	{
		throw OPCException("Failed to get IID_IOPCServer");
	}
	ATL::CComPtr<IOPCServer> iOpcServer ( reinterpret_cast<IOPCServer*>(mqi.pItf) );
