	...
	/* [iid_is][out] */ void **ppvObject )
{
	...
	//OPCServer *server = new frl::opc::OPCServer();
	
	if ( !server )
	{
		server = new frl::opc::OPCServer();
		server->AddRef();
	}	
...
void OPCServerFactory::SendShutdownRequestToClient()
{
	server->SendShutdownRequestToClient();
}

void OPCServerFactory::ShutdownServer()
{
	if (server)
	{
		server->Release();
...
bool OPCServerFactory::hasConnectedClient()
{
	return server->hasConnectedClient();
}

bool OPCServerFactory::serverIsLive()
{
	return server != NULL;
}
