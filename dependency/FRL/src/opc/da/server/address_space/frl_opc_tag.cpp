...
void Tag::writeFromOPCClient( const os::win32::com::Variant &newVal )
{
	if( os::win32::com::Variant::isEqual( value, newVal ) )
		return;

	if ( is_before_opc_client_change_subscr_cb )
	{
		if ( value.getType() != VT_EMPTY )
		{
			if ( !before_opc__client_change_cb( this )) return;
		}
	}	
...
void Tag::writeForce( const os::win32::com::Variant &newVal )
{
	value = newVal;
	::GetSystemTimeAsFileTime( &timeStamp );
}
...
void Tag::subscribeToBeforeOpcClientChange( const boost::function< bool( const address_space::Tag* const ) > &function_ )
{
	FRL_EXCEPT_GUARD();
	if( is_opc_change_subscr_cb )
		FRL_THROW_S_CLASS( AlreadySubscribe );
	before_opc__client_change_cb = function_;
	is_before_opc_client_change_subscr_cb = True;
}
