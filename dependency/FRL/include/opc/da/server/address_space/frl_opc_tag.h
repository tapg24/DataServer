...
private:
	...
	Bool is_before_opc_client_change_subscr_cb;
	boost::function< bool( const address_space::Tag* const ) > before_opc__client_change_cb;
...
public:	
	...
	void writeFromOPCClient( const os::win32::com::Variant &newVal );

	void write( const os::win32::com::Variant &newVal );

	// для обновления значения и временного штампа тега, даже если значение не изменилось
	void writeForce( const os::win32::com::Variant &newVal );
	...
	void subscribeToBeforeOpcClientChange( const boost::function< bool( const address_space::Tag* const ) > &function_ );
};
